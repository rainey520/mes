package com.ruoyi.project.quality.mesInput.mapper;

import com.ruoyi.project.app.domain.Product;
import com.ruoyi.project.quality.mesInput.domain.MesInput;
import org.apache.ibatis.annotations.Param;

import java.util.List;

/**
 * mes录入记录 数据层
 * 
 * @author sj
 * @date 2019-11-12
 */
public interface MesInputMapper 
{
	/**
     * 查询mes录入记录信息
     * 
     * @param id mes录入记录ID
     * @return mes录入记录信息
     */
	public MesInput selectMesInputById(Integer id);
	
	/**
     * 查询mes录入记录列表
     * 
     * @param mesInput mes录入记录信息
     * @return mes录入记录集合
     */
	public List<MesInput> selectMesInputList(MesInput mesInput);
	
	/**
     * 新增mes录入记录
     * 
     * @param mesInput mes录入记录信息
     * @return 结果
     */
	public int insertMesInput(MesInput mesInput);
	
	/**
     * 修改mes录入记录
     * 
     * @param mesInput mes录入记录信息
     * @return 结果
     */
	public int updateMesInput(MesInput mesInput);
	
	/**
     * 删除mes录入记录
     * 
     * @param id mes录入记录ID
     * @return 结果
     */
	public int deleteMesInputById(Integer id);
	
	/**
     * 批量删除mes录入记录
     * 
     * @param ids 需要删除的数据ID
     * @return 结果
     */
	public int deleteMesInputByIds(String[] ids);

	/**
	 * 通过mes信息查询记录
	 * @param mesInfo 产品批次
	 * @return 结果
	 */
    MesInput selectMesInputByMesInfo(@Param("mesInfo") String mesInfo);

	/**
	 * 查询所有的mes录入记录列表
	 *
	 * @return mes录入记录集合
	 */
	List<MesInput> selectMesInputListAll();

	/**
	 * 查询所有的mes录入记录列表产品排序
	 *
	 * @return mes录入记录集合
	 */
	List<Product> selectMesInputListByPro();
}